...
import { getOrSetLang } from "@utils/helpers/locale";
...
export default function AddNoteButton ({ style, editingNote, setEditingNote }) {
  ...
  if (style === 2) {
    ...
    <div className="mt-16">
            <button
                className="min-w-48 px-6 py-3 bg-black rounded-2xl inline-flex justify-center transition-all items-center hover:shadow-[0px_2px_10px_0px_rgba(151,71,255,1.00)] transition-300ms gap-2.5"
                onClick={onClick}
  ...
  return (
    ...
    >
        <XIcon 
            className="w-8 h-8 mr-[10px] transition-all duration-300 group-hover:rotate-180 text-neutral-800 group-hover:text-[#e134fc]" 
        />
